...
namespace xkfy_mod
{
    public partial class EventChart : Form
    {
        ...
        List<EventData> _eventData = new List<EventData>();
        Dictionary<string, string> _isHaveEvent = new Dictionary<string, string>();
        ...
        private void EventChart_Load(object sender, EventArgs e)
        {
            ...
            _eventData = XmlHelper.XmlDeserializeFromFile<List<EventData>>(path, Encoding.UTF8);
            ...
            {
                foreach (EventData ed in _eventData)
                {
                    if(ed.Round == null)
                        continue;
                    if (!_isHaveEvent.ContainsKey(ed.Round))
                    {
                        _isHaveEvent.Add(ed.Round,ed.EventId);
                    }
                }
            }
            SetDg1BgColor();
        ...
        private void SetDg1BgColor()
        {
            foreach (DataGridViewRow dr in dg1.Rows)
            {
                if(dr.Cells["col_key"].Value == null)
                    return;
                if (_isHaveEvent.ContainsKey(dr.Cells["col_key"].Value.ToString()))
                {
        ...
        ExplainHelper _eh = new ExplainHelper();
        
        Dictionary<string, string> _dicEventId = new Dictionary<string, string>();
        ...
        private void SetParatNodes(DataRow drDq, TreeNode pNode, string text)
        {
            ...
            else
            {
                ...
                if (iDevelopType == "3")
                {
                    ...
                    string rewardData = _eh.ExplainRewardData(sRewardData);
                    ...
                    rewardData = _eh.ExplainRewardData(sRewardData);
                    talkid = DataHelper.GetId(rewardData);
        ...
        {
            string iId = iArg3;
            if(!_dicEventId.ContainsKey(iArg3))
                _dicEventId.Add(iArg3, iArg3);
            ...
            if (drTalks.Length > 0)
            {
                ...
                if (drTalk["bInFields"].ToString() == "1")
                {
                    ...
                    for (int i = 1; i < 5; i++)
                    {
                        ...
                        nodeLoop.Name = sbArg;
                        if (_dicEventId.ContainsKey(sbArg))
                        {
            ...
            {
                List<EventData> hxEve = _eventData.Where(f => f.Round != null && f.ParantId == iId).ToList();
        ...
        ToolsHelper _tl = new ToolsHelper();
        private void ExplainEvent(string id)
        {
            ...
            string[] explain = _tl.ExplainDevelopQuest(striCondition, striType, striArg1, striArg2);
        ...
        private void dg1_CurrentCellChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                var result = _eventData.Where(f => f.Round == key);
